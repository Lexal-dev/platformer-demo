export default class MovingPlatform extends Phaser.Physics.Arcade.Sprite {
    ...
    constructor(
      ...
      speed: number = 50,
      range: number = 50,
      isHorizontalDirection: boolean = true
    ) {
      ...
      // Vérification si 'body' est bien de type Body avant d'appeler setAllowGravity
      if (this.body instanceof Phaser.Physics.Arcade.Body) {
        this.body.setAllowGravity(false); // Utilisation correcte de setAllowGravity
      }
      ...
      this.range = range;
      this.speed = speed;
      this.maxRange = this.isHorizontalDirection ? this.startX + this.range : this.startY + this.range;
    ...
    private moving(delta: number): void {
      ...
      const accelerationFactor = Phaser.Math.Easing.Sine.Out(
        Math.min(this.elapsedTime / accelerationDuration, 1)
      );
